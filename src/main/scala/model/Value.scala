...
object Value {
  ...
  case class PairValue(first: (Int, List[Value] => Value, Value), second: (Int, List[Value] => Value, Value)) extends Value

  case class ClosureValue(code: List[Instruction]) extends Value
  
  implicit val valueOrdering: Ordering[Value] = (x: Value, y: Value) => (x, y) match {
